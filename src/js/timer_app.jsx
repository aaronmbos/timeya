...
const TimerCard = (props) => {
  const [timerState, setTimer] = useLocalStorage(props.id.toString(), {'lastTicked': 0, 'seconds': 0, 'isStarted': false, 'wasPaused': false, 'isEditable': props.type === 'countdown', 'editSeconds': 0});    
  ...
  const countdownTick = () => setTimer(() => {
    return {
      ...
      'seconds': timerState.lastTicked === 0 ? parseInt(timerState.seconds) + 1 : 
          timerState.wasPaused ? parseInt(timerState.seconds) : 
              parseInt(timerState.seconds) + (timerState.lastTicked - Math.floor(Date.now() / 1000)),
    }
  });
  ...
  const handleTimerReset = () => {
    setTimer({'lastTicked': 0, 'seconds': 0, 'isStarted': false, 'wasPaused': false, 'isEditable': props.type === 'countdown', 'editSeconds': 0});
    clearInterval(interval);
  }
        ...
        />
        {
          ...
          <div className={`card-footer ${props.name ? '' : 'hide'}`}>
            <div className='card-footer-item'>
            {
              ...
              <i id={`reset-${props.id}`} onClick={handleTimerReset} className={`fas fa-redo reset-button ${timerState.seconds > 0 || timerState.isComplete  ? '' : 'hide'}`}></i>
            }
            </div>
        </div>
        }
  ...
  return (
    <div className='card-content'>
      {props.isEditable ? 
        ...
        <span onClick={props.handleTimerClick} className='timer-click content is-centered'>
          {getTimeFromSeconds()}
        </span>
      }
    </div>
